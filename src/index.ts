...
window.onload = async () => {
  ...
  function render(time) {
    ...
    const eye = m4.transformPoint(m4.rotateZ(m4.identity(), time), [
      1,
      300,
      200
    ]);
...
function logBuffer(gl: WebGL2RenderingContext) {
  gl.flush();
  let data = new Uint8Array(100 * 100 * 4);
  gl.readBuffer(gl.COLOR_ATTACHMENT2);
  gl.readPixels(0, 0, 100, 100, gl.RGBA, gl.UNSIGNED_BYTE, data);
  console.log(data);
}
